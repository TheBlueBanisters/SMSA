...
# CH-SIMS 数据集预处理脚本
# 使用方法:
#   1. 直接运行（使用默认路径）: bash preprocess_chsims.sh
#   2. 指定输入目录: bash preprocess_chsims.sh --input ./path/to/chsims
#   3. 指定输出目录: bash preprocess_chsims.sh --output ./path/to/output
#   4. 同时指定: bash preprocess_chsims.sh --input ./chsims --output ./data/chsims_new

# 配置参数
DATASET="chsims"
INPUT_DIR="./chsims"                    # CH-SIMS原始数据目录
OUTPUT_DIR="./data/chsims_processed"    # 处理后的数据输出目录
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --output|-o)
            ...
            ;;
        --help|-h)
            echo "用法: bash preprocess_chsims.sh [选项]"
            echo ""
            echo "选项:"
            echo "  --input, -i DIR    CH-SIMS原始数据目录（默认：./chsims）"
            echo "  --output, -o DIR   输出目录（默认：./data/chsims_processed）"
            echo "  --help, -h         显示帮助信息"
            echo ""
            echo "示例:"
            echo "  bash preprocess_chsims.sh"
            echo "  bash preprocess_chsims.sh --input ./chsims_raw --output ./data/chsims_v2"
            exit 0
            ;;
        *)
            echo "未知选项: $1"
