...
BATCH_SIZE="32"          # 批大小，留空使用默认值（chsims:32, chsimsv2:32, meld:16）⭐ 增大到16减少过拟合
LEARNING_RATE="5e-5"        # 学习率，留空使用默认值（chsims:1e-4, chsimsv2:1e-4, meld:5e-5）
NUM_EPOCHS="50"         # 训练轮数，留空使用默认值（chsims:50, chsimsv2:50, meld:40）⭐ 增加到50，配合早停
HIDDEN_DIM=""           # 隐藏层维度（默认：256）
DROPOUT="0.3"              # Dropout率（默认：0.1）
EARLY_STOP_PATIENCE="5" # 早停等待轮数（留空使用默认10，当前3，设为0禁用）⭐
EARLY_STOP_METRIC="acc_2"    # 早停监控指标（留空使用默认mae，当前acc_2）⭐
...
NO_KEY_FRAME_SELECTOR=false    # 关闭关键帧选择
